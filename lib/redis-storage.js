...
function createRedisStorage({ url, token }) {
  ...
  async function command(args) {
    ...
    if (!response.ok) {
      throw new Error(`Upstash ${args[0]} 실패 (${response.status}): ${text.slice(0, 300)}`);
    }
    let data;
    try {
      data = JSON.parse(text);
    } catch (error) {
      throw new Error(`Upstash 응답 파싱 실패 (${args[0]}): ${text.slice(0, 300)}`);
    }
    if (data && data.error) {
      throw new Error(`Upstash ${args[0]} 오류: ${data.error}`);
    }
    return data.result;
  }
